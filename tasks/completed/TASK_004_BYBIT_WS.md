...
| `docs/DATA_CONTRACT.md` | Â§2 (WS endpoint), Â§3.3 (candle types â€” CRITICAL), Â§7 (freshness), Â§10 (failure modes) |
| `docs/SYSTEM_ARCHITECTURE.md` | Â§3.1 (MarketDataProvider interface) |
| `docs/TEST_SPEC.md` | INT-003, INT-004, REL-002, REL-003 |
| `src/scanner/config.py` | `bybit_testnet`, `environment` |
| `src/scanner/models.py` | `Candle` â€” especially `is_closed` field |
...
If T003 is not yet merged, coordinate â€” do not duplicate those files.
...
src/scanner/config.py          â€” do not modify
src/scanner/models.py          â€” do not modify
src/scanner/market_data/bybit_rest.py â€” T003 scope; do not modify
src/scanner/market_data/models.py     â€” T003 scope; read only
...
### R-001 â€” WebSocket Endpoint Configuration
...
# bybit_testnet=False requires environment='live' â€” raise RuntimeError otherwise.
```

### R-002 â€” BybitWebSocketClient Class
...
class BybitWebSocketClient:
    ...
    async def run_forever(self) -> None:
        # Main loop: connect â†’ subscribe â†’ receive â†’ reconnect on failure
...
3. Set `is_closed` from the message's `"confirm"` field:
   - `"confirm": true` â†’ `is_closed = True` (candle just closed)
   - `"confirm": false` â†’ `is_closed = False` (candle updating mid-period)
4. **Validate OHLC integrity only** â€” do NOT check `is_closed` at this layer:
   - OHLC violation or zero/negative price â†’ discard; log **CRITICAL** (per Â§10)
   - Other OHLC/volume failures â†’ discard; log **ERROR** (per Â§8)
   - `is_closed` is intentionally excluded from WS-layer validation â€” forming
     candles (`is_closed=False`) are valid transport updates and must be emitted
5. **Call `on_candle(candle)`** with the result â€” both forming and closed candles

> **Architecture note:** The `is_closed == True` rule in DATA_CONTRACT.md Â§8 applies
> to the **strategy pipeline** (T008+), not to the transport layer.
> The WebSocket client is a transport â€” it emits faithfully.
...
The WebSocket client emits ALL updates â€” filtering is the caller's responsibility.

### R-004 â€” Reconnect Logic
...
On disconnect:
  ...
  2. Wait: exponential backoff â€” 1s, 2s, 4s, 8s, max 30s
...
If pong not received within 5s: treat as disconnect â†’ reconnect (R-004)
```

### R-006 â€” Stale Stream Detection (`stale_detector.py`)
...
INFO:     Candle received â€” symbol, interval, is_closed, close_price
INFO:     Reconnected â€” topics re-subscribed
WARNING:  Disconnect detected â€” reason, reconnect attempt N
WARNING:  Stale topic â€” symbol, interval, seconds_since_last_message
ERROR:    Candle validation failure (general) â€” symbol, field, value
ERROR:    BTC 4H topic stale â€” regime data at risk
ERROR:    Candle callback raised exception â€” exception type and message
CRITICAL: OHLC violation (high < low, etc.) â€” symbol, field, value
CRITICAL: Price = 0 or negative â€” symbol, field, value
```

> **Correction (2026-08-31):** "Invalid candle discarded â€” WARNING" was a CTO authoring
> error. DATA_CONTRACT.md Â§10 (higher authority) specifies CRITICAL for OHLC violations
> and zero/negative prices. DATA_CONTRACT.md Â§8 specifies ERROR for other validation
> failures. Specific cases in Â§10 override the general rule in Â§8.

### R-008 â€” Callback Exception Isolation
...
- `bybit_ws_kline_message.json` â€” forming candle WS message (`"confirm": false`)
- `bybit_ws_kline_closed.json` â€” closed candle WS message (`"confirm": true`)
...
```
Task:       T004 â€” Bybit WebSocket Client
Agent:      CODEX
...
Requirements Completed:  [R-001 âœ… through R-010 âœ…]
...
Recommended Next Step:   T005 (CandleStore) â€” after T003 also complete
```
...
- Forming candle passed through (not filtered) â€” filtering is T005's job
...
After T004 approval: **SKILL CANDIDATE** â€” deferred.
...
*End of Task Contract â€” T004 Bybit WebSocket Client*
